...
// Please only uses this for PMNet setup
///////////////////////////////////////////
#include <iostream>
#include <fstream>
using namespace std;
...
size_t generateReadRequest(char* appBuff, int seqno, uint32_t* hashInput){
    size_t retSize = 0;
    unsigned buffPtr=0;
    // Write the header of the command
    // 3 args for the command, key and value.
...
//    usedkeys_vec[randIdx].c_str();
    sscanf(usedkeys_vec[randIdx].c_str(), "%d", hashInput);
    //cerr << hashInput[0] << endl;
    char keyString[20];
