...
#define PMSWITCH_OPCODE_CACHE_RESPONSE      0x06
...
size_t pmSwitchEncapsulate(char* output, uint8_t type, uint16_t session_id, uint32_t seq_no, char* payload, size_t payload_length, uint32_t hashedAddr){
    struct pmswitchHeader pmswitch_hds;
    pmswitch_hds.seq_no = seq_no;
    pmswitch_hds.session_id = session_id;
    pmswitch_hds.type = type;
    // Need to change here!
    //cerr << "PMAddrInput:" << hashedAddr << endl;
...
size_t pmSwitchEncapsulate(char* output, uint8_t type, uint16_t session_id, uint32_t seq_no, char* payload, size_t payload_length){
    return pmSwitchEncapsulate(output, type, session_id, seq_no, payload, payload_length, seq_no);
}
